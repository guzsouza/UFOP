...
int main() {
    ...
    for(int i = 0; i < quantDocs; i++) {
        ...
        for(int j = 0; j < quantChaves; j++)
            if(!insereDocumento(tabelaHash, chaves[j], nomeDoc)) 
                printf("ERROR: Problema pra adicionar chave\n");
    }
    ...
    //esvazia
    for(int i = 0; i < M; i++) {
        Celula* atual = tabelaHash[i].cabeca->prox;
        while (atual != NULL) {
            Celula* prox = atual->prox;
            free(atual);
            atual = prox;
        }
        free(tabelaHash[i].cabeca);
    }
