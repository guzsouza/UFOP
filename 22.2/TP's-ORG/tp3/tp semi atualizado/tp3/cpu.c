...
void start(Machine* machine, Instruction* instructions, int* memoriesSize) {
    startHD(&machine->hd, memoriesSize[0]);
...
void executeInstruction(Machine* machine, int PC, int Map_Type) {
    ...
    switch (instruction.opcode) {
        ...
        case 3: //interrupção
            ...
            //vetor de instruções auxiliar, para guardar as instruções antes da interrupção
            Instruction *aux = (Instruction*) malloc(machine->NumInstructions * sizeof(Instruction));
            aux = machine->instructions;

            //vetor de instruções da interrupção
            Instruction *InterruptionVet = (Instruction*) malloc(4 * sizeof(Instruction));
            machine->instructions = InterruptionVet;

            //abertura de arquivo
            FILE *InstArq;
            int line = 0;
            InstArq = fopen("interruption.in", "r");

            //caso o arquivo não possa ser aberto, o programa alertará o usuário do erro e prosseguir.
            if (InstArq == NULL){
                printf("Não foi possível abrir o arquivo de instruções\n");
                break;
            }
            ...
            //lendo as informações do arquivo de interrupção e passando pra máquina
            for (int index = 0; index < 21; index++) {

                fscanf(InstArq, "%d", &line);
                int position = index % 7;

                //printf("a instrução %d, tem o valor de:%d na posição %d\n\n", indexArq, line, position + 1);

                if (position == 0)
                    machine->instructions[indexArq].add1.block = line;
                else if (position == 1)
                    machine->instructions[indexArq].add1.word = line;
                else if (position == 2)
                    machine->instructions[indexArq].add2.block = line;
                else if (position == 3)
                    machine->instructions[indexArq].add2.word = line;
                else if (position == 4)
                    machine->instructions[indexArq].add3.block = line;
                else if (position == 5)
                    machine->instructions[indexArq].add3.word = line;
                else if (position == 6)
                    machine->instructions[indexArq].opcode = line;

                //executando as intruções
                executeInstruction(machine, indexArq, MAP_TYPE);
            ...
            //após executar as intruções, os valores antigos (antes de ocorrer a interrupção), voltaram pra máquina
            machine->instructions = aux;

            // Fecha o arquivo
            fclose(InstArq);

            //liberando a memória extra utilizada
            free(InterruptionVet);
