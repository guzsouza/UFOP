...
Instruction* generateRandomInstructions(int hdSize) {
    ...
    for (int i=0; i<n; i++) {
        instructions[i].add1.block = rand() % hdSize;
        instructions[i].add1.word = rand() % WORDS_SIZE;
        instructions[i].add2.block = rand() % hdSize;
        instructions[i].add2.word = rand() % WORDS_SIZE;
        instructions[i].add3.block = rand() % hdSize;
        instructions[i].add3.word = rand() % WORDS_SIZE;
        instructions[i].opcode = rand() % 4;
    }
...
Instruction* readInstructions(char* fileName, int* memoriesSize, int* NumInstructions) {
