...
int memoryRamMapping(MemoryBlock* RAM, int tam, int block, int* lpos) {
    for(int i = 0; i < tam; i++)
        if(RAM[i].words == block) {
            *lpos = i;
            return 1;
        }
    return 0;
...
Line* MMUSearchOnMemorys(Address add, Machine* machine, int Map_Type) {
    ...
    // } else if (cache3[l3pos].tag == add.block) { 
        ...
        updateMachineInfos(machine, &(cache// 3[l3pos]));
        return &(cache3[l3pos]);
        ...
        l2pos = mapping(add.block, &machine->l2, Map_Type); /* Need to check the position of the block that will leave the L2 */
        l3pos = mapping(add.block, &machine->l3, Map_Type); /* Need to check the position of the block that will leave the L3 */
