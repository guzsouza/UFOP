...
void start(Machine* machine, Instruction* instructions, int* memoriesSize) {
    startMemoriaExterna(memoriesSize[0]));
...
void executeInstruction(Machine* machine, int PC, int Map_Type) {
    ...
    switch (instruction.opcode) {
        ...
        case 3: // Interruption
            ...
            // le interruption.in e passa para as variaves
            ...
            // pega uma lista de instruucoes // instrucoes.txt
            // cria uma machine
            
            // roda um start(machine, instruções, tamHD)
